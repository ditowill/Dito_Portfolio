...
# Cabang dengan pendapatan terbesar
...
# Cabang dengan pendapatan terkecil
...
#Menggabungkan umur dengan performa cabang
...
df_loan_mei_flag <- df_loan_mei_umur %>%
  group_by(umur) %>%mutate(Q1 = quantile(total_amount, 0.25),Q3 = quantile(total_amount, 0.75), IQR = (Q3-Q1)) %>%mutate(flag = ifelse(total_amount < (Q1 - IQR), 'rendah','baik'))

df_loan_mei_flag %>% filter(flag == 'rendah') %>%
  mutate_if(is.numeric, funs(comma))


#Plot cabang dengan performa rendah
...
# Perbandingan performa cabang pada umur yang sama
...
#Perbandingan performa agen pada cabang yang memiliki performa rendah
...
#Perbandingan performa agen pada cabang yang paling baik dalam 3 bulan
